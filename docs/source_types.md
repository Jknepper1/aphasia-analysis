...
The transcripts are then sent through the following process in order to be prepared for the TTS model which requires plantext sentences with not CHAT formatted notations.
...
**Currently Broca, Wernicke, Anomic, and Control data are used in this project. A selection of a MAXIMIUM 60 sample files is taken from each of these categories to be utilized as the base for the following data types found in `_talkbank_sorted_flo`**
...
These are more for comparing BatchAlign as we use it to their Human Coded counterparts
